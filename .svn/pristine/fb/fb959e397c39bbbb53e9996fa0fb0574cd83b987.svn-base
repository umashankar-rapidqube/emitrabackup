...
public class BhamashahCardDaoImpl implements BhamashahCardDao {
	...
	public JSONObject getIFSCDetails(String ifscCode) {
		...
		try {
            ...
            logger.debug("Response status of selectBankDetails:" + serviceResponse.getStatus());
            logger.info("Response status of selectBankDetails");
            ...
            logger.error("BhamashahCardDaoImpl, Exception in selectBankDetails : " + ex.getMessage());
        }
	...
	public void insertEntryIntoLog(JSONObject responseObj) {
		...
		try {
            ...
            logger.debug("Response status of insertMobileOTPInformation:" + serviceResponse.getStatus());
            logger.info("Response status of insertMobileOTPInformation");
        } catch (Exception ex) {
            logger.error("BhamashahCardDaoImpl, Exception in insertMobileOTPInformation : " + ex.getMessage());
        }
	}
	...
	public String insertUidAuthDetails(JSONObject serviceDataJson) {
		...
		try {
            ...
            DBServiceResponse serviceResponse = dbHTTPUtils.pullDataFromDBApiServer(queryDetails);
            logger.debug("Response status of insertBhamashahPaymentDetails:" + serviceResponse.getStatus());
            logger.info("Response status of insertBhamashahPaymentDetails");
            if (HttpStatus.SC_OK == serviceResponse.getStatus()) {
                JSONArray resultSet = serviceResponse.getJsonArray();
                if (null != resultSet && resultSet.length() > 0) {
                	org.json.JSONObject rs = (org.json.JSONObject) resultSet.get(0);
            ...
            logger.error("BhamashahCardDaoImpl, Exception in insertBhamashahPaymentDetails : " + ex.getMessage());
        }
	...
	public String updateBhamashahPaymentDetails(HttpServletRequest request , JSONObject memberData) {
		...
		try {
            ...
            DBServiceResponse serviceResponse = dbHTTPUtils.pushDataToDBApiServer(queryDetails);
            logger.debug("Response status of updateBhamashahPaymentDetails:" + serviceResponse.getStatus());
        } catch (Exception ex) {
            logger.error("BhamashahCardDaoImpl, Exception in updateBhamashahPaymentDetails : ", ex);
        }
	...
	public String saveCertificateDetails(HttpServletRequest request , JSONObject serviceDataJson) {
		...
		try {
            ...
            DBServiceResponse serviceResponse = dbHTTPUtils.pullDataFromDBApiServer(queryDetails);
            logger.debug("Response status of saveCertificateDetails:" + serviceResponse.getStatus());
            if (HttpStatus.SC_OK == serviceResponse.getStatus()) {
                JSONArray resultSet = serviceResponse.getJsonArray();
                if (null != resultSet && resultSet.length() > 0) {
                	org.json.JSONObject rs = (org.json.JSONObject) resultSet.get(0);
            ...
            logger.error("BhamashahCardDaoImpl, Exception in saveCertificateDetails : ", ex);
        }
		return transID;
	}
	...
	@Override
	public org.json.JSONObject getUpdateDetails(String procName , String transId , String action) {
		// TODO Auto-generated method stub
		
		org.json.JSONObject responseJson = new org.json.JSONObject();
		try {
            QueryDetails queryDetails = new QueryDetails(procName);
            queryDetails.setAutoCommit(false);
            ...
            DBServiceResponse serviceResponse = dbHTTPUtils.pullDataFromDBApiServer(queryDetails);
            logger.debug("Response status of getUpdateDetails:" + serviceResponse.getStatus());
            if (HttpStatus.SC_OK == serviceResponse.getStatus()) {
                JSONArray resultSet = serviceResponse.getJsonArray();
                if (null != resultSet && resultSet.length() > 0) {
                	org.json.JSONObject rs = (org.json.JSONObject) resultSet.get(0);
            ...
            logger.error("BhamashahCardDaoImpl, Exception in getUpdateDetails : ", ex);
        }
	...
	public void updateBhamashahStatus(String transId, String status) {
		...
		try {
            ...
            logger.error("BhamashahCardDaoImpl, Exception in insertBhamashahPaymentDetails : ", ex);
        }
	}
}