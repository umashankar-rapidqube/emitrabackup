...
public class MobileOTP {
	...
	final static int MAX_SIZE = 200000;
	...
	public JSONObject sendMessageOnMobile(String mobileNo) {
		...
		try {
			...
			logger.error("sendMessageOnMobile, Exception msg :: Message Not Sent!Please input valid Details :"
					+ e.getMessage());
			/* e.printStackTrace(); */
			return null;
		}
	...
	public static HttpURLConnection sendSingleSMS(HttpURLConnection lconnection, String username, String password,
			String senderId, String mobileNo, String message) {

		logger.info("sendSingleSMS");
		logger.info("sendSingleSMS");
		try {
			logger.info("**************");
			logger.debug("Username = " + username + "Password = " + password + "senderId = " + senderId + "mobileNo = "
			...
			String query = "username=" + URLEncoder.encode(username) + "&password=" + URLEncoder.encode(password)
					...
					+ "&mobileno=" + URLEncoder.encode(mobileNo) + "&senderid=" + URLEncoder.encode(senderId);

			logger.info("Printing query = ");
			logger.debug("Printing query = " + query);
			...
			lconnection.setRequestProperty("User-Agent", "Mozilla/4.0 (compatible; MSIE 5.0; Windows 98; DigExt)");

			// open up the output stream of the connection
			output = new DataOutputStream(lconnection.getOutputStream());

			// write out the data
			output.writeBytes(query);

			// get ready to read the response from the cgi script
			input = new DataInputStream(lconnection.getInputStream());
			...
			logger.error("Something bad just happened.");
			logger.error("Something bad just happened." + e.getMessage());
			logger.error(e);
			/* e.printStackTrace(); */
		} finally {
			try {
	...
	public static HttpURLConnection sendSMS(String mobilenumber, String msg) throws MalformedURLException, IOException {
		...
		if (connection != null) {
			logger.info("Mobile NUmber length  for Sending SMS: ");
			logger.debug("Mobile NUmber length  for Sending SMS: " + mobilenumber.length());
