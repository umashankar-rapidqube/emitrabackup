...
public class AgricultureController {
	...
	public void getNoteDetails(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		try {
			/*
			 * logger.info("NoteDetails, total10note :"+NoteType.
			 * total10Note+" total20note :"+NoteType.total20Note+" total50note:"+NoteType.
			 * total50Note
			 * +" total100note :"+NoteType.total100Note+" total200note :"+NoteType.
			 * total200Note+" total500note :" +
			 * NoteType.total500Note+" total2000note :"+NoteType.total2000Note );
			 */

			/*
			 * NoteType.total10Note++; NoteType.total20Note++; NoteType.total50Note++;
			 * NoteType.total100Note++; NoteType.total200Note++; NoteType.total500Note++;
			 * NoteType.total2000Note++;
			 */

			int total = (NoteType.total10Note * 10) + (NoteType.total20Note * 20) + (NoteType.total50Note * 50)
					+ (NoteType.total100Note * 100) + (NoteType.total200Note * 200) + (NoteType.total500Note * 500)
					+ (NoteType.total2000Note * 2000);
			...
			logger.error("AgricultureController, Exception in getNoteDetails : " + e.getMessage());
			/* e.printStackTrace(); */
		} finally {
			pw.print(jarr);
			pw.flush();
